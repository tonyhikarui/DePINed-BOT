...
class Register:
    def __init__(self):
        ...
        self.ref_codes = []
        self.ref_code_index = 0
        self.used_ref_codes = set()
    ...
    def load_ref_code(self):
        try:
            with open('ref_code.txt', 'r') as f:
                # Load all ref codes and filter out empty lines
                self.ref_codes = [line.strip() for line in f if line.strip()]
                if not self.ref_codes:
                    print(f"{Fore.RED}No valid referral codes found in ref_code.txt{Style.RESET_ALL}")
                    return None
                print(f"{Fore.GREEN}Loaded {len(self.ref_codes)} referral codes{Style.RESET_ALL}")
                return self.get_next_ref_code()
        except FileNotFoundError:
            print(f"{Fore.RED}File ref_code.txt not found{Style.RESET_ALL}")
            return None

    def get_next_ref_code(self):
        """Get next unused referral code"""
        attempts = 0
        while attempts < len(self.ref_codes):
            ref_code = self.ref_codes[self.ref_code_index]
            self.ref_code_index = (self.ref_code_index + 1) % len(self.ref_codes)
            
            if ref_code not in self.used_ref_codes:
                return ref_code
            attempts += 1
        
        print(f"{Fore.RED}All referral codes have been used{Style.RESET_ALL}")
        return None
    ...
    def register_user(self, email, password):
        ...
        try:
            ...
            if response.status_code == 400:
                print(f"{Fore.YELLOW}HTTP 400 error detected, saving to need_confirm.txt{Style.RESET_ALL}")
                self.save_to_file("./result/need_confirm.txt", f"{email}|{password}")
                return "need_confirm"  # Special return value for 400 error
            ...
            try:
                ...
                while not confirm or not confirm.get('data', {}).get('token'):
                    if retry_count >= max_retries:
                        ...
                        self.save_to_file("./result/fail_register.txt", f"{account['email']}|{account['password']}|{token}|500_error")
                        break
                    
                    ref_code = self.get_next_ref_code()
                    if not ref_code:
                        print(f"{Fore.RED}No more referral codes available{Style.RESET_ALL}")
                        break
                        
                    confirm = self.confirm_user_ref(token, ref_code)
                    if confirm and confirm.get('data', {}).get('token'):
                        self.used_ref_codes.add(ref_code)  # Mark code as used
                        self.save_to_file("./result/used_refs.txt", f"{ref_code}|{account['email']}")
                        break

                    if not confirm:
                        retry_count += 1
                        if 'HTTP Error 500:' in str(confirm):  # Check specifically for 500 error
                            print(f"{Fore.YELLOW}HTTP 500 error detected, saving to fail_register.txt{Style.RESET_ALL}")
                            self.save_to_file("./result/fail_register.txt", f"{account['email']}|{account['password']}|{token}|500_error")
                            break
                ...
                if confirm and confirm.get('data', {}).get('token'):
                    ...
                    if not confirm:  # If not already saved due to 500 error
                        self.save_to_file("./result/fail_register.txt", f"{account['email']}|{account['password']}|{token}|other_error")
